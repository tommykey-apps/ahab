...
コンテナ操作は start / stop / restart / 削除のみ。
...
- start / stop / restart / 削除（実行中は強制削除の確認を挟む）
- ログ追従
- イメージ一覧（タグ / ID / サイズ / 作成日時 / 使用中のコンテナ）と削除
- ボリューム一覧（名前 / ドライバ / 作成日時 / 使用中 / 使用中のコンテナ）と削除
...
- `none` ネットワークは描かない（孤立したコンテナは箱だけ置く）。`host` ネットワークはホスト公開ポートと同じ host ノードに矢印なしで結ぶ
- 公開ポートは IPv4 / IPv6 の重複を除いて 1 本にする
